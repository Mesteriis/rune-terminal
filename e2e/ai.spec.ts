...
test('AI provider settings show gateway telemetry after a mocked Codex run', async ({ page, request }) => {
  ...
  await expect(page.getByText(/^Probed .* provider\.$/)).toBeVisible()
})
...
test('AI sidebar /run preserves persisted remote host semantics from the selected context widget', async ({
  ...
  await expect(page.getByText('Executed `df -h` on the selected PVE terminal.')).toBeVisible()
  expect(streamRequests).toEqual([])
  expect(pageErrors).toEqual([])
})
...
test('AI sidebar executes approved terminal prompts in the selected context widget instead of the active terminal', async ({
  ...
  await page.getByRole('button', { name: 'Send prompt' }).click()
  await expect(page.getByRole('button', { name: 'Approve' })).toBeVisible()
  await page.getByRole('button', { name: 'Approve' }).evaluate((node) => {
...
test('AI sidebar restores persisted remote context before approved terminal execution after reopen', async ({
  page,
  request,
}) => {
  test.setTimeout(180_000)
  ...
  await page.route('**/api/v1/agent/terminal-commands/plan', async (route) => {
    ...
    await route.fulfill({
      ...
      json: {
        command: 'df -h',
        summary: 'Check free disk space on the restored remote host context.',
      },
      status: 200,
    })
  })
      ...
      json: {
        conversation: {
          ...
          messages: [
            ...activeConversation.messages,
            {
              id: `msg-remote-restore-${Date.now()}`,
              role: 'assistant',
              content: 'Executed `df -h` on the restored PVE terminal context.',
              status: 'complete',
              provider: 'stub',
              model: 'stub-model',
              created_at: new Date().toISOString(),
...
test('AI sidebar confirms and retries approval-required terminal execution on the selected SSH-backed context', async ({
  page,
  request,
}) => {
  test.setTimeout(180_000)
      ...
      json: {
        conversation: {
          ...
          messages: [
            ...activeConversation.messages,
            {
              id: `msg-remote-approval-${Date.now()}`,
              role: 'assistant',
  ...
  await expect
    .poll(() => toolBodies.length, { timeout: 30_000 })
    .toBe(3)

  expect(toolBodies).toMatchObject([
    {
      tool_name: 'term.send_input',
      input: {
        append_newline: true,
  ...
  await expect(page.getByText('Approved and ran `df -h` on the selected PVE terminal.')).toBeVisible()
  expect(streamRequests).toEqual([])
  expect(pageErrors).toEqual([])
})
