...
LOCAL_TASK_CONTROL_TOKEN ?= runa-local-dev-task-token

MACOS_ARCH ?= arm64
LINUX_ARCH ?= amd64

SPLIT_DEV_ENV := LOCAL_BACKEND_LISTEN=$(LOCAL_BACKEND_LISTEN) LOCAL_BACKEND_URL=$(LOCAL_BACKEND_URL) LOCAL_FRONTEND_HOST=$(LOCAL_FRONTEND_HOST) LOCAL_FRONTEND_PORT=$(LOCAL_FRONTEND_PORT) LOCAL_AUTH_TOKEN=$(LOCAL_AUTH_TOKEN) LOCAL_TASK_CONTROL_TOKEN=$(LOCAL_TASK_CONTROL_TOKEN)
BACKEND_WATCH_ENV := LOCAL_BACKEND_LISTEN=$(LOCAL_BACKEND_LISTEN) LOCAL_BACKEND_URL=$(LOCAL_BACKEND_URL) LOCAL_AUTH_TOKEN=$(LOCAL_AUTH_TOKEN) LOCAL_TASK_CONTROL_TOKEN=$(LOCAL_TASK_CONTROL_TOKEN)
BACKEND_RUN_ENV := RTERM_AUTH_TOKEN=$(LOCAL_AUTH_TOKEN) RTERM_TASK_CONTROL_TOKEN=$(LOCAL_TASK_CONTROL_TOKEN)
