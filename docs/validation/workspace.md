...
- Static validation confirmed the commander pending bar now accepts inline rename input through the shared `Input` primitive, `F2` is wired into the commander keyboard and footer hints, and the fake-client/store layer now exposes overwrite-aware `copy/move/rename` paths instead of relying only on silent unique-name generation.
- Browser-level rename/overwrite smoke was attempted against a local dev server on `http://127.0.0.1:4200`, but the automation environment did not complete that run cleanly because the local browser tooling was already locked by another profile/session. This slice therefore claims `lint:active` + `build` validation and static code validation for rename/overwrite, but not a fresh browser-confirmed rename/overwrite walkthrough yet.
