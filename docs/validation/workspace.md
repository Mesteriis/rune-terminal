...
- The new commander confirm bar is still frontend-only. It does not yet claim backend-backed approvals, server-enforced policy checks, rename prompts, overwrite prompts, or cross-widget transfer semantics.
- Commander history is still frontend-only and fake-client backed. It does not yet claim persisted pane history across app restarts or backend-driven path validation beyond the local fake client.
