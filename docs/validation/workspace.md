...
- Scope:
  ...
  - the top shell `AI` toggle now uses the dedicated button logo asset `frontend/assets/icon/button/ai-agent.png`, and the image fills the button without inner padding
...
- Static validation confirmed the top shell `Toggle AI panel` control now renders the shared `Avatar` component with `frontend/assets/icon/button/ai-agent.png`, and the button/image styles remove the old inner padding so the logo fills the control chrome.
