...
- Scope:
  ...
  - the top shell `AI` toggle now uses the dedicated button logo asset `frontend/assets/icon/button/ai-agent.png`, and the button chrome itself is flattened so the logo sits without inner padding or button border
...
- Static validation confirmed the top shell `Toggle AI panel` control now renders the shared `Avatar` component with `frontend/assets/icon/button/ai-agent.png`, and the button style overrides remove the old inner padding, border, shadow, and background chrome around that logo.
