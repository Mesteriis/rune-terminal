...
- Scope:
  ...
  - the AI header logo now comes from a reusable shared `Avatar` component backed by a local `runa-avatar.svg` asset instead of the old generic sparkles icon
...
- Static validation confirmed the shared UI layer now also exposes a reusable `Avatar` component built from `Box`, and the frontend source tree now includes `src/vite-env.d.ts` so Vite asset imports like `runa-avatar.svg` resolve cleanly under strict TypeScript.
...
- This avatar replacement pass is validated by type-check and production build only. A fresh browser-level confirmation of the new `Runa avatar` asset in the header is not claimed here because the follow-up Playwright smoke against the active Vite session did not complete cleanly in this environment.
