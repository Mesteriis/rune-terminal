...
- The active runtime still does not include `ollama`, the earlier internal AI proxy draft, or a broad provider/API-key universe.
  ...
  - richer last-error taxonomy (`missing_binary`, `auth_required`, `timeout`, `unreachable`, `model_unavailable`, `invalid_config`, `upstream_rejected`) instead of a generic failure bucket
  ...
  - the AI shell header and provider settings now choose operator retry actions from that same gateway snapshot:
...
- A focused localization validation pass on `2026-05-01` confirmed the AI composer, conversation navigator, collapsed AI sidebar, route chrome, and composer shortcut settings now read visible UI copy from locale tables for `en`, `ru`, `zh-CN`, and `es`. The pass ran `npm --prefix frontend run test -- src/widgets/ai/ai-composer-widget.test.tsx src/widgets/ai/ai-panel-header-widget.test.tsx src/app/app-ai-sidebar.test.tsx` (44 passed; existing jsdom canvas warnings only) and `npm --prefix frontend run lint:active`.
