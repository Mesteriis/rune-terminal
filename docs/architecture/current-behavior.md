...
- The terminal shell is now much closer to TideTerm's compact term surface, has an in-panel search row backed by the xterm search addon, and can host multiple backend-owned sessions inside one widget with explicit active-session switching; it still lacks TideTerm's richer multi-session sidebar/tmux-style browsing surface.
