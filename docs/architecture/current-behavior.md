...
- Conversation stream cancellation appends a core-owned
  `agent.conversation.cancel_stream` audit event with the stream id and
...
- The active provider path now also has a backend-owned gateway telemetry surface instead of a revived standalone proxy stack:
  ...
  - that same gateway snapshot also owns runtime route readiness (`route_status_state`, `route_status_message`, `resolved_binary`, checked-at, probe latency)
  - `GET /api/v1/agent/providers` is configuration truth only; it no longer doubles as runtime readiness
...
- Streaming assistant output is active through `POST /api/v1/agent/conversation/messages/stream`; OpenAI-compatible HTTP sources now forward provider SSE `delta.content` chunks as text deltas, while Codex CLI and Claude Code CLI still emit buffered output as one final text delta. The AI composer exposes an explicit `Cancel response` control for cancellable chat streams; cancellation aborts the browser request and keeps partial assistant output visible locally as cancelled/error state.
...
- SSH profile management currently supports direct saved host/user/port/identity-file fields plus a narrow one-way `~/.ssh/config` import route for `Host`, `HostName`, `User`, `Port`, and `IdentityFile`. The settings shell exposes a `Remote` section that can create, edit, delete, list, preflight-check, and mark saved SSH profiles as the default target, and can also trigger the import route. It does not negotiate richer auth flows, expand proxy topologies, or provide long-lived remote status tracking.
