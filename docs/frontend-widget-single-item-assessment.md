...
## Latest Execution Boundary Check: `telemetryrequired`

- Current parent import path in use:
  - `frontend/ui/widgets/RTAIPanelWidget/aipanel.tsx` imports `TelemetryRequiredMessage` from `./telemetryrequired`.
- Current export/import path to preserve: `./telemetryrequired` (relative local path used by parent remains unchanged).
- Direct imports outside `RTAIPanelWidget`: none found in `frontend/app`, `frontend/ui`, or `frontend/wave.ts`.
- Dedicated SCSS state: no dedicated `telemetryrequired` SCSS file exists today; this execution slice must introduce `telemetryrequired.style.scss`.
- Boundary confirmation: this execution slice stays inside the approved leaf boundary (`telemetryrequired` local files plus minimal local import rewiring only inside `RTAIPanelWidget`).

## Post-byokannouncement remaining RTAIPanelWidget assessment
...
- Completed local contract sub-slices excluded:
  ...
  - `aifeedbackbuttons*`
  - `byokannouncement*`
- Remaining files assessed in this pass: **20**
...
|---|---|---|---|---|
| `ai-utils.ts` | runtime/action logic | depended by `aipanel*`, `aimessage`, `aimode`, `waveai-model`, `compat-conversation`, `aidroppedfiles` | No | Shared helper spine; large blast radius. |
| `aidroppedfiles.tsx` | possible leaf UI candidate | imports `ai-utils`, `waveai-model`; used by `aipanel.tsx` and `aipanel-compat.tsx` | Partially | UI leaf shape, but mutates attachment/model state and has two orchestration dependents. |
...
| `aimode.tsx` | conversation/model/state logic | imports `ai-utils`, `waveai-model`; used by `aipanel.tsx`, `aipanelmessages.tsx` | No | Mode switching tied to settings, telemetry, and model state. |
| `aipanel-compat.tsx` | compat bridge | imports many local modules; used via `aipanel.tsx` | No | Primary compat surface with run-command/conversation adaptation paths. |
| `aipanel-contextmenu.ts` | runtime/action logic | imports `compat-context`, `waveai-model`; used by `aipanel.tsx`, `aipanelheader.tsx`, `aipanel-compat.tsx` | No | Shared action path across core orchestration files. |
...
| `aipanelinput.tsx` | conversation/model/state logic | imports `ai-utils`, `waveai-focus-utils`, `waveai-model`; used by `aipanel.tsx`, `aipanel-compat.tsx`, `waveai-model.tsx` | No | Input/focus pipeline with shared state coupling. |
...
| `aitooluse.tsx` | runtime/action logic | imports `aitypes`, `restorebackupmodal`, `waveai-model`; used by `aimessage.tsx` | No | Tool restore/highlight actions coupled to model-side effects. |
...
| `compat-context.ts` | compat bridge | used by `waveai-model.tsx`, `aipanel.tsx`, `aipanel-contextmenu.ts` | No | Runtime compat context boundary. |
| `compat-conversation.ts` | compat bridge | imports `ai-utils`, `aitypes`; used by `aipanel-compat.tsx` | No | Conversation mapping core for compat flow. |
| `restorebackupmodal.tsx` | runtime/action logic | imports `aitypes`, `waveai-model`; used by `aitooluse.tsx` | Partially | Modal UI around restore state machine and model actions. |
...
| `run-command.ts` | runtime/action logic | imports `aitypes`; used by `aipanel-compat.tsx` and tests | No | Command execution/approval core. |
| `telemetryrequired.tsx` | possible leaf UI candidate | imports `waveai-model` + telemetry RPC clients; used by `aipanel.tsx` | Partially | Mostly isolated UI card with single action path and one parent dependent. |
