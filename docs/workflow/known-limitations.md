...
- backend AI conversations are now persisted as explicit DB-backed threads with per-conversation CLI session continuity, but conversation management is still minimal: create + switch only, with no rename/archive/delete/search UX yet
