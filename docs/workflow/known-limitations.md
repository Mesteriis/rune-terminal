...
- desktop runtime metadata writes are now also durable against torn-file startup races: the desktop shell writes both `~/.rterm/runtime.json` and `~/.rterm/settings.json` through an atomic temp-file rename path, and startup now also deletes malformed `settings.json` payloads instead of silently reusing default settings while leaving broken metadata on disk; broader crash-recovery policy is still intentionally incomplete
