...
- desktop runtime startup and the narrow `SIGTERM` shutdown path are now hardened: `npm run tauri:dev` recovers stale watcher attachments, startup failures no longer leak a freshly spawned backend, and sending `SIGTERM` to `rterm-desktop` now tears down the desktop-owned backend and watcher before exit; broader desktop runtime hardening (single-instance policy, richer crash recovery, port-policy cleanup) is still intentionally incomplete
