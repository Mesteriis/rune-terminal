...
- backend AI conversations are now persisted as explicit DB-backed threads with per-conversation CLI session continuity, but conversation management is still intentionally narrow: create + switch + active-thread rename only, with no archive/delete/search UX yet
