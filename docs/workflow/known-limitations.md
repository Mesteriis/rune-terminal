...
- backend AI conversations are now persisted as explicit DB-backed threads with per-conversation CLI session continuity, and the shell navigator now supports local filtering over the loaded thread list plus active-thread rename/archive/restore/delete, but conversation management is still intentionally narrow: there is no broader conversation search, archive-management view, or multi-panel conversation UX yet
