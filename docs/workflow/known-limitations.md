...
- backend AI conversations are now persisted as explicit DB-backed threads with per-conversation CLI session continuity, and the shell navigator now supports local filtering over the loaded thread list, but conversation management is still intentionally narrow: create + switch + active-thread rename + local filter only, with no archive/delete lifecycle UX yet
