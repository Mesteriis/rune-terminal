...
- full TideTerm AI/chat panel behavior
...
| AI panel | Left-side AI/chat panel with header, messages, input, mode/context controls | Left panel exists, but it is still an operator/control surface, not a real chat-first AI panel | Missing message timeline, chat input, widget-context semantics, and known TideTerm AI flows | Rebuild TideTerm AI panel behavior on top of the new tool runtime and policy model | Keep policy/runtime explicit; do not port old AI/backend entanglement | `missing` |
