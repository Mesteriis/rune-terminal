...
| AI panel | Left-side AI/chat panel with header, messages, input, mode/context controls | Left panel now follows the TideTerm panel grammar more closely: TideTerm-shaped header with widget-context toggle and overflow menu, top mode strip, welcome state, and a runtime-backed transcript for quick actions, approvals, and posture changes | Still missing real AI conversation transport, file attach flows, richer message/tool rendering, and a true composer flow | Continue porting the imported TideTerm AI panel structure while rebinding actions to the new runtime and policy surfaces | Keep policy/runtime explicit; do not port old AI/backend entanglement | `partial` |
