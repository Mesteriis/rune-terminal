...
| Tabs | Real tab bar with active tab, pinned tabs, add tab flows, AI toggle, drag behavior | Workspace now exposes a real tab model and the top strip binds to `tabs` and `active_tab_id` instead of pretending widgets are tabs | Missing creation/pinning/ordering/drag semantics and richer tab content ownership | Continue expanding the new tab model until the top shell no longer relies on widget-derived approximations | Do not port TideTerm's full frontend store graph; port behavior only | `partial` |
