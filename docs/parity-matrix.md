...
| --- | --- | --- | --- | --- | --- | --- |
| App shell behavior | App-shaped shell with top tab bar, left AI panel, center content, right slim widgets/settings rail | TideTerm-derived shell is now present with left AI panel, center terminal stage, widget-first dock, and a tabbar-shaped header with workspace switcher and AI control cluster | Shell framing is close, but tab controls are still reduced and there is no full workspace switcher behavior yet | Keep copying TideTerm shell placement and panel behavior before adding any new visual concepts | Preserve React + local loopback transport; do not reintroduce old global shell state | `partial` |
...
| Tabs | Real tab bar with active tab, pinned tabs, add tab flows, rename, AI toggle, drag behavior | Workspace now exposes a real tab model, the top strip binds to `tabs` and `active_tab_id`, and the shell can create, close, rename, pin, unpin, and drag-reorder terminal tabs inside their current group | Missing richer drag polish, cross-group drag behavior, manual ordering affordances beyond drag, and richer tab content ownership | Continue expanding the new tab model until the top shell no longer relies on reduced controls | Do not port TideTerm's full frontend store graph; port behavior only | `partial` |
| Widgets | Slim right-side widgets/apps/settings/help surface with secondary actions | Right rail is now widget-first again, with widget focus in the main stack and settings/audit/tools demoted to footer actions | Still missing richer widget catalog, add-widget flows, and app/help surfaces | Continue replacing temporary controls with TideTerm-derived widget/app flows as features land | Keep dock as adapter to explicit runtime/UI state, not a global action bucket | `partial` |
