...
| Tabs | Real tab bar with active tab, pinned tabs, add tab flows, AI toggle, drag behavior | Workspace now exposes a real tab model, the top strip binds to `tabs` and `active_tab_id`, and the shell can create and close terminal tabs | Missing pinning, ordering, drag semantics, rename, and richer tab content ownership | Continue expanding the new tab model until the top shell no longer relies on reduced controls | Do not port TideTerm's full frontend store graph; port behavior only | `partial` |
