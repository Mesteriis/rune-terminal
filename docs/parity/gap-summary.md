...
- Streaming AI response breadth (`UX`): OpenAI-compatible text deltas and frontend cancel are live, but CLI token/reasoning/tool-call streaming remains `PARTIAL`.
- Terminal advanced affordances/sidebar (`UX`): single-session search is in place, but multi-session sidebar and deeper session browsing remain `PARTIAL`.
