...
| Advanced remote breadth: session groups, tmux resume, richer SSH topology, `wsh` replacement | `Planned` | The terminal runtime now supports grouped sessions inside one widget with explicit active-session switching, saved SSH profiles can opt into tmux-backed resume through the same backend-owned SSH launch path, and the `Remote` settings shell can browse discovered tmux sessions, bind one back into the profile editor, or open/resume that session directly into the active workspace. ADR 0028 records the broader remote breadth v2 as a later extension of that same model. | The remaining gap is a richer tmux session manager/catalog plus broader auth/topology breadth; WSL is explicitly optional and only reopens with broader platform scope. |
