...
| `.ssh/config` import | `Partially transferred` | One-way import now supports direct `Host` / `HostName` / `User` / `Port` / `IdentityFile` profile fields, `Include`, wildcard-host defaults, and `Match host/originalhost` when deriving concrete saved profiles. | `ProxyJump`, broader `Match` criteria, keychain/passphrase workflows, and reverse sync are still absent. |
