...
| Явный `/run <command>` execution path | FULL | `core/app/ai_terminal_command.go`; `core/transport/httpapi/handlers_agent_conversation.go`; `frontend/app/aipanel/run-command.ts`; `frontend/app/aipanel/aipanel-compat.tsx` | `VERIFIED`: active compat AI panel detects `/run`, executes `term.send_input` through `POST /api/v1/tools/execute`, and renders the observed result in the current transcript. |
| Объяснение результата terminal command | FULL | `core/app/ai_terminal_command.go`; `core/transport/httpapi/handlers_agent_conversation.go`; `frontend/app/aipanel/run-command.ts`; `frontend/app/aipanel/aipanel-compat.tsx` | `VERIFIED`: active compat AI panel calls `POST /api/v1/agent/terminal-commands/explain` after `/run` execution and appends the backend assistant reply in the same transcript. |
| Approval внутри AI/tool flow | FULL | `core/app/tool_policy.go`; `core/toolruntime/*`; `frontend/app/aipanel/run-command.ts`; `frontend/app/aipanel/run-command-approval.tsx`; `frontend/app/aipanel/aipanel-compat.tsx` | `VERIFIED`: active compat AI panel surfaces approval-required `/run`, confirms via `safety.confirm`, retries with `approval_token`, and keeps audit-visible `approval_used:true` on the approved retry path. |
