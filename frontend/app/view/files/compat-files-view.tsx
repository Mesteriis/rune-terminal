...
export function CompatFilesView({ widgetId, path, connectionId }: CompatFilesViewProps) {
    ...
    return (
        <div className="flex h-full min-h-0 flex-col gap-3 bg-black/20 p-3" data-testid={`compat-files-widget-${widgetId}`}>
            <div className="flex items-center justify-between gap-2">
                <div>
                    <div className="text-xs uppercase tracking-wide text-cyan-200">Files block</div>
                    <div className="text-[11px] text-secondary break-all">{currentPath || path}</div>
                </div>
                <div className="flex items-center gap-2">
                    <span className="text-[10px] uppercase tracking-wide text-secondary">
                        {remoteDirectory ? "remote source" : "local source"}
                    </span>
                        <button
                            type="button"
            ...
            {loading ? (
                ...
                <div className="rounded border border-red-500/40 bg-red-500/10 px-3 py-2 text-sm text-red-200 whitespace-pre-wrap">
                    {loadError}
                </div>
            ) : (
