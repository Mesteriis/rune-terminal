...
export class TermWshClient extends WshClient {
    ...
    async handle_termgetviewport(
        rh: RpcResponseHelper,
        data: any
    ): Promise<{ linestart: number; lineend: number; lines: string[] }> {
        const termWrap = this.model.termRef.current;
        if (!termWrap || !termWrap.terminal) {
            return { linestart: 0, lineend: 0, lines: [] };
        }
        const buffer = termWrap.terminal.buffer.active;
        const startLine = buffer.viewportY;
        const endLine = Math.min(buffer.length, startLine + termWrap.terminal.rows);
        const lines: string[] = [];
        for (let i = startLine; i < endLine; i++) {
