...
import { getApi, refocusNode } from "@/app/store/global";
...
export class TabClient extends WshClient {
    ...
    async handle_workspacefocuswidget(
        rh: RpcResponseHelper,
        data: { blockid: string }
    ): Promise<{ focused: boolean; widget_id: string; block_id: string }> {
        if (!data?.blockid) {
            throw new Error("blockid is required");
        }
        refocusNode(data.blockid);
        return {
            focused: true,
            widget_id: data.blockid.slice(0, 8),
