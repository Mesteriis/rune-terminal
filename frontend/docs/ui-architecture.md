...
- `AiPanelHeaderWidget` now also owns the shell-visible conversation navigator chrome: active-thread summary trigger, recent conversation menu, local search/filter over the loaded conversation list, `New` action, inline active-thread rename, and delete-confirm UX over the backend conversation list/create/activate/rename/delete contract, without moving conversation semantics into the widget layer.
