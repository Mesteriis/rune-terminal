...
- Commander keyboard handling is now being decomposed into focused pure handlers: file-dialog blocking, `Alt+Left/Right` history, ctrl-modifier shortcuts, and pending-operation key flows now live in `keyboard-handlers.ts` with direct unit coverage, while typeahead and the main navigation switch still remain in `keyboard.ts`.
