...
- Commander blocked/non-text file dialogs now also keep backend `Open externally` handoff state visible inside the dialog after a successful `/api/v1/fs/open` request, instead of closing immediately and leaving the operator without any confirmation that the OS-opener handoff was dispatched.
