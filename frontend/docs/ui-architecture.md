...
- `TerminalDockviewHeaderActionsWidget` keeps terminal-group close controls inside the compact Dockview chrome only; creating sibling terminal sessions belongs to `TerminalWidget` and is exposed through a separate terminal header action group, so Dockview tabs stay at the widget/panel boundary.
...
- `DialogPopup` provides the stateless shared dialog surface, including the wide settings-dialog variant and an optional body-content slot for host-provided settings widgets. The settings variant keeps only the icon close control and a layout-only body slot, leaving settings title/description and footer actions out of the shared chrome.
...
- `SettingsShellWidget` now owns the shell-wide settings navigation inside the existing modal body, keeping `General`, `AI`, `Terminal`, `Remote`, `MCP`, and `Commander` as explicit sections without forking the shared dialog chrome; the `AI` subtree can collapse instead of permanently occupying vertical space, while the shell itself now presents a compact scrollable section navigator and a single active-section content surface without repeating modal, sidebar, and section descriptions or drawing an additional outer frame.
...
- `AgentProviderSettingsWidget` now lives under `AI > Провайдеры AI` inside that shell settings surface and supports an embedded rendering mode with flatter container framing by composing `ClearBox` for layout-only wrappers; the embedded toolbar exposes only actions because the shell already owns the active-section title, and run diagnostics stay opt-in until the operator selects a saved run. The active provider setup surface now covers Codex CLI, Claude Code CLI, and one narrow OpenAI-compatible HTTP source kind, while `features/agent/api/provider-client.ts`, `features/agent/model/provider-settings-draft.ts`, and `features/agent/model/use-agent-provider-settings.ts` keep backend transport, draft serialization, and editor state outside the shared modal host.
...
- `AppAiSidebar` now also owns the shell-only `collapsed / expanded` presentation state for the visible AI work panel: app-level open/close still lives in `$isAiSidebarOpen`, while the sidebar itself reopens in a collapsed summary mode with route/thread/mode status and expands locally without changing backend conversation/runtime truth.
...
- `AiPanelHeaderWidget` keeps provider-route telemetry and panel-mode switching in the visible shell header for fast operator access, while the conversation navigator stays focused on thread discovery and thread lifecycle actions.
- `AppAiSidebar` shell framing now leaves the header popover path unclipped (`frame/content overflow visible`, body clipped separately), so the AI conversation navigator can layer above the panel body without changing backend/runtime semantics.
...
- `AiComposerWidget` keeps the expanded drafting surface on the same compact two-column control grid, so provider/model/tuning/context remain drafting controls around the textarea instead of widening back into a separate hero toolbar.
...
- Terminal Dockview tabs now use a shortened compact `cwd` label as the primary visible title, keep the full `cwd` in the title tooltip, and limit active-tab meta to connection/session pills while the `New session` action stays in a terminal header action group and the session rail renders only compact focus/close session tabs when more than one sibling session exists.
