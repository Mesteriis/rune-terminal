...
- `TerminalDockviewHeaderActionsWidget` keeps terminal-group close controls inside the compact Dockview chrome only; creating sibling terminal sessions belongs to `TerminalWidget`'s in-panel session rail, so Dockview tabs stay at the widget/panel boundary.
...
- Terminal Dockview tabs now use a shortened compact `cwd` label as the primary visible title, keep the full `cwd` in the title tooltip, and limit active-tab meta to connection/session pills while the `New session` action stays inside the terminal widget's session rail.
