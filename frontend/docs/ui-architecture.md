...
- `TerminalWidget` renders the terminal-specific body composition for terminal panels and now owns the in-panel terminal chrome composition: live `TerminalStatusHeader`, a restart action slot, `TerminalToolbar`, and the xterm-backed `TerminalSurface`.
