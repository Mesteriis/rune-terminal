...
- `AiPanelHeaderWidget` now also owns the shell-visible conversation navigator chrome: active-thread summary trigger, recent conversation menu, and `New` action over the backend conversation list/create/activate contract, without moving conversation semantics into the widget layer.
