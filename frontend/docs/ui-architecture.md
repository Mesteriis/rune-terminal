...
- the token layer owns dark/light environment overrides through the app theme root attributes (`data-runa-theme` / `data-runa-resolved-theme`) with `prefers-color-scheme` as the system fallback, while `src/index.css` applies the corresponding shell/body print flattening rules
...
- `TerminalToolbar` keeps search ergonomics ahead of decorative status chrome: when terminal search opens, the search row takes over the trailing utility area, and the terminal renderer mode no longer claims a dedicated visible badge in the widget/header chrome.
...
- Commander model orchestration now also keeps pure view-model projection in
  `features/commander/model/view-model.ts` and F3/F4 file-dialog IO in
  `features/commander/model/file-dialog-controller.ts`, so
  `useCommanderWidget` no longer owns those surfaces inline while backend
  transport still stays behind `features/commander/api/client.ts`.
- `TerminalWidget` renders the terminal-specific body composition for terminal panels and now treats the single-tab Dockview header as the primary terminal chrome surface: live status/meta, toolbar affordances, and terminal actions stay on one compact line, while the body focuses on the xterm-backed `TerminalSurface`; multi-tab groups still fall back to body-local chrome instead of introducing a second backend transport model.
- that single-tab terminal header now presents an operator-facing `term_N` display alias instead of the raw backend widget id, while keeping runtime `widget_id` semantics unchanged underneath; the same compact header also splits terminal identity, status pills, and action controls into separate surface groups so status chrome does not share one underlay with button controls.
- `TerminalStatusHeader` keeps the shell pill backend-driven: local sessions can open the runtime shell list and request a validated restart with the selected shell, while SSH sessions keep the pill as read-only connection metadata.
- `TerminalWidget` reads `useAppLocale()` and resolves visible action/session/search copy plus terminal AI handoff prompt templates through `terminal-widget-copy.ts` for every advertised shell locale (`ru`, `en`, `zh-CN`, `es`), so active terminal chrome and terminal-originated AI instructions participate in the shell localization contract instead of hard-coding strings in the widget body.
- `TerminalDockviewTabWidget` renders terminal-specific Dockview tab chrome for terminal panels and now expands into the primary widget header when the group has a single terminal tab, carrying terminal identity/meta, toolbar actions, and shell controls in one compact line instead of repeating them inside the body.
...
- Dockview overflow affordances for terminal groups are also themed as part of that same system in `src/index.css`; the trigger/dropdown use the `--runa-dockview-overflow-*` semantic tokens so compressed terminal tabs do not fall back to generic or dark-only Dockview chrome.
...
- `TerminalSettingsSection` now owns the `Settings -> Terminal` slice and reads/writes terminal font size, line height, theme mode, scrollback, plus cursor behavior through the runtime-owned terminal settings contract (`shared/api/terminal-settings.ts` plus `features/terminal/model/use-terminal-preferences.ts`), including a one-shot restore-to-defaults action that still persists through the same contract instead of inventing a reset-specific transport path. Its visible copy now reads from the shared locale contract through `terminal-settings-section-copy.ts`, so terminal settings participate in the same `ru` / `en` / `zh-CN` / `es` shell localization path as General settings.
- `RuntimeSettingsSection` now owns the live `General` content inside that shell and reads the real desktop runtime contract for `watcher_mode` plus bootstrap context, while still degrading honestly to a read-only browser fallback during the split `make dev` loop where Tauri-only mutation is unavailable. It also exposes the app-owned shell theme preference (`system`, `light`, `dark`) backed by `AppThemeProvider`, which projects the selected theme onto document root attributes consumed by the token layer.
...
- `AppAiSidebar` now also owns the shell-only `collapsed / expanded` presentation state for the visible AI work panel: app-level open/close still lives in `$isAiSidebarOpen`, while the sidebar itself reopens in a collapsed summary mode and expands locally without changing backend conversation/runtime truth.
...
- In the expanded state, `AiPanelHeaderWidget` now reads as operator tooling rather than hero chrome: route telemetry, thread actions, and mode switching live inside the navigator surface, while the visible shell header stays compact and notebook-like.
...
- `AiComposerWidget` keeps the expanded notebook state on the same compact two-column control grid, so provider/model/tuning/context remain drafting controls around the textarea instead of widening back into a separate hero toolbar.
...
- `TerminalSurface`, `ModalHostWidget`, and `WidgetBusyOverlayWidget` now receive external mount/theme targets through props, so those lower layers no longer query for modal anchors or Dockview group wrappers on their own. `TerminalSurface` also receives the resolved app theme signal from `TerminalWidget`, allowing live xterm palettes to be re-applied after root theme changes.
...
- The shell environment layer now also exposes app-owned theme defaults: `AppThemeProvider` persists `system` / `light` / `dark`, writes `data-runa-theme` and `data-runa-resolved-theme` on `document.documentElement`, and the token layer still uses `prefers-color-scheme: light` as the no-JS/system fallback. `@media print` flattens shell chrome so Dockview surfaces print without glow, blur, or resize affordances.
...
- `TerminalToolbar` adds the terminal-local addon controls layer for search, clipboard actions, and jump/clear viewport affordances without exposing a separate renderer-status badge.
...
- `TerminalDockviewTabWidget` keeps terminal-specific Dockview composition in the widget layer, reusing `TerminalStatusHeader` in both compact and expanded single-tab modes so the terminal identity can move into Dockview header chrome without forking backend semantics.
...
- The same component-layer pattern now also covers `TerminalToolbar`, so its toolbar/search chrome lives in `terminal-toolbar.styles.ts` and can be mounted either in the Dockview single-tab header or in the terminal body fallback path without duplicating the control contract.
