...
- `TerminalWidget` reads `useAppLocale()` and resolves visible action/session/search copy through `terminal-widget-copy.ts` for every advertised shell locale (`ru`, `en`, `zh-CN`, `es`), so active terminal chrome participates in the shell localization contract instead of hard-coding labels in the widget body.
