...
- `TerminalSettingsSection` now owns the `Settings -> Terminal` slice and reads/writes terminal font size through the runtime-owned terminal settings contract (`shared/api/terminal-settings.ts` plus `features/terminal/model/use-terminal-preferences.ts`), so the shell keeps live xterm density in sync with backend state instead of treating font size as frontend-local storage; deeper terminal settings (`theme`, `scrollback`, `line height`) remain out of contract for now.
