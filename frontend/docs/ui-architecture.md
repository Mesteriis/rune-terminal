...
- `TerminalWidget` renders the terminal-specific body composition for terminal panels and now owns the in-panel terminal chrome composition: live `TerminalStatusHeader`, `TerminalToolbar`, and the xterm-backed `TerminalSurface`.
...
- `TerminalWidget` stays in the widget layer and now owns the terminal body composition only: status header, toolbar search/clipboard affordances, and renderer surface wiring against the existing terminal session contract.
