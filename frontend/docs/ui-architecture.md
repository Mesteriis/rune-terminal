...
- Commander widget runtime state now persists per `widgetId` in `localStorage`, so reload restores pane paths, history stacks, cursor/selection, and view toggles without introducing a second frontend-owned filesystem model; legacy `client.directories` payloads are read only as a compatibility input during restore.
