...
- `AiPanelHeaderWidget` now also owns the shell-visible conversation switcher chrome: active conversation select plus `New` action over the backend conversation list/create/activate contract, without moving conversation semantics into the widget layer.
