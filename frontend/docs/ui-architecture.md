...
- Commander files now also support backend `F3/F4` flows: `F3` opens a read-only preview modal backed by `/api/v1/fs/read`, `F4` opens a UTF-8 text editor modal backed by `/api/v1/fs/file`, and `Ctrl+S` persists the active buffer back through the backend before the pane reloads the saved entry metadata.
