...
- `TerminalSettingsSection` now owns the `Settings -> Terminal` slice and keeps terminal font size explicitly frontend-local through `features/terminal/model/use-terminal-preferences.ts`, so the shell can tune live xterm density without pretending a backend terminal settings contract already exists.
