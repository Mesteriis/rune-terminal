...
- `TerminalSettingsSection` now owns the `Settings -> Terminal` slice and reads/writes terminal font size, line height, plus theme mode through the runtime-owned terminal settings contract (`shared/api/terminal-settings.ts` plus `features/terminal/model/use-terminal-preferences.ts`), so the shell keeps live xterm density and palette mode in sync with backend state instead of treating those values as frontend-local storage; deeper terminal settings (`scrollback`, cursor behavior) remain out of contract for now.
