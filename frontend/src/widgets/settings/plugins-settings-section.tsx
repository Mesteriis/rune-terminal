...
const pluginsSettingsCopy: Record<AppLocale, PluginsSettingsCopy> = {
  ...
  ru: {
    ...
    actor: (username) => `пользователь: ${username}`,
    catalogDescription: 'Включайте, выключайте, обновляйте или удаляйте плагины без перезапуска приложения.',
    catalogTitle: 'Каталог',
    description:
      'Локальный каталог плагинов хранится на стороне приложения. Источники установки ограничены URL git-репозитория и ZIP-архива.',
    disable: 'Выключить',
    ...
    errorDuplicateMetadataKey: (key) => `Повторяющийся ключ метаданных: ${key}`,
    errorInstall: 'Не удалось установить плагин',
    errorLoad: 'Не удалось загрузить установленные плагины',
    errorMetadataFormat: 'Строки метаданных должны быть в формате `key=value`.',
    errorMetadataKey: 'Ключи метаданных не должны быть пустыми.',
    filterAria: 'Фильтр установленных плагинов',
    filterPlaceholder: 'Фильтр по плагину, владельцу, источнику, инструменту или метаданным',
    gitRefAria: 'Git-ссылка плагина',
    gitRefPlaceholder: 'Необязательная ветка или тег',
    gitRepository: 'Git-репозиторий',
    installedCount: (count) => `${count} установлено`,
    installing: 'Установка…',
    installFrom: (kind) => `Установить из ${kind}`,
    installDescription: 'Поля доступа уже сохраняются для будущего контроля прав, но пока не применяются.',
    installedMessage: (name) => `Установлен ${name}.`,
    installTitle: 'Установка пакета плагина',
    loadingCatalog: 'Загрузка каталога плагинов…',
    metadataAria: 'Метаданные плагина',
    metadataCount: (count) => `метаданные ${count}`,
    ...
    ownerMeta: (owner, visibility, metadataCount) =>
      [
        `владелец: ${owner}`,
        visibility,
        metadataCount && metadataCount > 0 ? `метаданные ${metadataCount}` : null,
      ]
        .filter(Boolean)
        .join(' · '),
    readyStatus: 'готов',
    remove: 'Удалить',
    removedMessage: (name) => `Удалён ${name}.`,
    ...
    toolCount: (count) => `${count} ${count === 1 ? 'инструмент' : 'инструментов'}`,
    update: 'Обновить',
