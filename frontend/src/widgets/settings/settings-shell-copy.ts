...
export const settingsShellCopy: Record<AppLocale, SettingsShellCopy> = {
  ...
  ru: {
    aiLimits: {
      activeBadge: 'active',
      description:
        'Сейчас этот surface показывает только готовность каналов для чата. Реальные quota и rate-limit появятся отдельным backend-owned шагом.',
      loading: 'Загружаю каталог провайдеров…',
      noProviders: 'Провайдеров пока нет.',
      routeActive: 'Активный провайдер для текущего chat runtime.',
      routeAuthRequired: 'Нужен локальный CLI login, прежде чем маршрут станет доступен.',
      routeConfiguredHttp:
        'HTTP source настроен. Используй refresh models, чтобы проверить удалённый каталог.',
      routeDisabled: 'Провайдер выключен; чат не будет отправлять сюда запросы.',
      routeDisabledHttp: 'HTTP source выключен; чат не будет отправлять сюда запросы.',
      routeReady: 'Backend-owned route готов к chat traffic.',
      routeUnchecked: 'Маршрут ещё не проходил probe.',
    ...
    aiModels: {
      ...
      enabledModelDescription: 'Включай модель, чтобы она появилась в dropdown основного AI чата.',
      ...
      noDiscoveredModels: 'Для этого провайдера backend пока не вернул модели.',
      noProviders:
        'Прямых AI-провайдеров пока нет. Сначала добавь CLI или HTTP source в разделе `Установленные приложения`.',
      providerAvailable: 'Доступен для активации в provider catalog.',
      providerActive: 'Сейчас активен в runtime.',
      refreshModels: 'Обновить модели',
      sectionDescription:
        'Переключай провайдера, чтобы посмотреть auto-discovered catalog, доступный для выбора в основном чате.',
      routeNotConfigured: 'URL OpenAI-compatible source ещё не настроен.',
      routeNotProbedClaude: 'Claude Code CLI route ещё не проходил probe.',
      routeNotProbedCodex: 'Codex CLI route ещё не проходил probe.',
    ...
    commander: {
      description:
        'Этот раздел зарезервирован для commander-specific options, когда они будут выведены из widget-local state.',
      notConnected: 'Отдельный конфигурационный surface для commander ещё не подключён.',
      planned: 'Planned',
      preferencesTitle: 'Commander preferences',
    ...
    sections: {
      general: {
        navTitle: 'Основные',
        navDescription: 'Режим runtime, язык shell и bootstrap context.',
        shellTitle: 'Основные',
        shellDescription: 'Жизненный цикл desktop runtime, выбор языка и текущий shell bootstrap context.',
        groupLabel: 'General',
      },
      'ai-apps': {
      ...
      'ai-models': {
        ...
        shellDescription:
          'Каталог моделей, которые backend вернул для активных провайдеров, и их экспозиция в основном AI чате.',
        groupLabel: 'AI',
      },
      'ai-composer': {
        navTitle: 'Composer',
      ...
      'ai-limits': {
        navTitle: 'Лимиты',
        navDescription: 'Готовность route и будущие quota surfaces.',
      ...
      terminal: {
        ...
        shellTitle: 'Terminal',
        shellDescription:
          'Отдельная точка входа для terminal runtime и будущих terminal preferences, без смешения с общими или AI настройками.',
        groupLabel: 'Runtime',
      },
      remote: {
        navTitle: 'Remote',
        navDescription: 'SSH профили и import из config.',
        shellTitle: 'Remote',
        shellDescription:
          'Backend-owned SSH profiles, узкий ~/.ssh/config import и явные ограничения вокруг advanced remote topology.',
        groupLabel: 'Runtime',
      },
      mcp: {
        navTitle: 'MCP',
        navDescription: 'Регистрация внешних серверов и lifecycle.',
        shellTitle: 'MCP',
        shellDescription:
          'Регистрируй remote MCP endpoints и управляй lifecycle через backend-owned MCP runtime без неявной AI context injection.',
        groupLabel: 'Runtime',
      },
      plugins: {
        navTitle: 'Plugins',
        navDescription: 'Локальный каталог и install lifecycle.',
        shellTitle: 'Plugins',
        shellDescription:
          'Backend-owned локальный plugin catalog с явными git/zip install sources, runtime-safe activation checks и future-facing access metadata.',
        groupLabel: 'Runtime',
      },
      commander: {
        navTitle: 'Commander',
        ...
        shellTitle: 'Commander',
        shellDescription:
          'Навигационный surface для file manager и dual-pane поведения. Commander-specific options будут появляться здесь по мере вывода их из widget-local state.',
        groupLabel: 'Workspace',
      },
    },
    sidebar: {
      description:
        'Общий навигатор по shell, AI runtime, terminal, remote, MCP, plugins и commander surface.',
      product: 'Rune Terminal',
      settings: 'Настройки',
    },
  },
