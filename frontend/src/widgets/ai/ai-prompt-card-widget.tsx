...
import type { AiPromptCardState } from '@/features/agent/model/types'
import {
    ...
    <RunaDomScopeProvider component={`ai-prompt-card-${prompt.id}`}>
      ...
      >
        <Box runaComponent={`ai-prompt-card-${prompt.id}-header`} style={aiPromptCardHeaderStyle}>
          <Box
            runaComponent={`ai-prompt-card-${prompt.id}-title-cluster`}
            style={aiPromptCardTitleClusterStyle}
          >
            <Text runaComponent={`ai-prompt-card-${prompt.id}-title`} style={aiPromptTitleStyle}>
              {prompt.title}
            </Text>
        ...
        {!isExpanded ? (
          <Text runaComponent={`ai-prompt-card-${prompt.id}-preview`} style={aiPromptCardPreviewStyle}>
            {activeSnapshot.preview}
          </Text>
        ) : (
          <Box
            runaComponent={`ai-prompt-card-${prompt.id}-expanded-body`}
            style={aiPromptCardExpandedBodyStyle}
          >
            <Box
              runaComponent={`ai-prompt-card-${prompt.id}-prompt-section`}
              style={aiPromptPromptSectionStyle}
            >
              <Box
                runaComponent={`ai-prompt-card-${prompt.id}-prompt-section-header`}
                style={aiPromptSectionHeaderStyle}
              >
                <Text
                  runaComponent={`ai-prompt-card-${prompt.id}-prompt-label`}
                  style={aiPromptCardSectionLabelStyle}
                >
                  Prompt
                </Text>
                <IconButton
              ...
              <Text
                runaComponent={`ai-prompt-card-${prompt.id}-prompt-text`}
                style={aiPromptCardSectionTextStyle}
              >
                {activeSnapshot.prompt}
              </Text>
            </Box>
            <Box
              runaComponent={`ai-prompt-card-${prompt.id}-reasoning-section`}
              style={aiPromptReasoningSectionStyle}
            >
              <Text
                runaComponent={`ai-prompt-card-${prompt.id}-reasoning-label`}
                style={aiPromptCardSectionLabelStyle}
              >
                Reasoning
              </Text>
              <Box
                runaComponent={`ai-prompt-card-${prompt.id}-reasoning-list`}
                style={aiPromptReasoningListStyle}
              >
                {activeSnapshot.reasoning.map((reasoningStep, index) => (
                  <Box
                    key={`${prompt.id}-reasoning-${index}`}
                    runaComponent={`ai-prompt-card-${prompt.id}-reasoning-item-${index + 1}`}
                    style={aiPromptReasoningItemStyle}
                  >
                    <Badge
                      runaComponent={`ai-prompt-card-${prompt.id}-reasoning-index-${index + 1}`}
                      style={aiPromptReasoningIndexStyle}
                    >
                      {index + 1}
                    </Badge>
                    <Text
                      runaComponent={`ai-prompt-card-${prompt.id}-reasoning-text-${index + 1}`}
                      style={aiPromptReasoningTextStyle}
                    >
                      {reasoningStep}
                    </Text>
                  </Box>
            ...
            {activeSnapshot.approvals?.length ? (
              <Box
                runaComponent={`ai-prompt-card-${prompt.id}-approval-section`}
                style={aiPromptApprovalSectionStyle}
              >
                <Text
                  runaComponent={`ai-prompt-card-${prompt.id}-approval-label`}
                  style={aiPromptCardSectionLabelStyle}
                >
                  Approval queue
                </Text>
                <Box
                  runaComponent={`ai-prompt-card-${prompt.id}-approval-list`}
                  style={aiPromptApprovalListStyle}
                >
                  {activeSnapshot.approvals.map((approval) => (
                    <Box
                      key={approval.id}
                      runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}`}
                      style={aiPromptApprovalRowStyle}
                    >
                      <Box
                        runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}-meta`}
                        style={aiPromptApprovalMetaStyle}
                      >
                        <Box
                          runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}-title-wrap`}
                          style={{ display: 'flex', alignItems: 'center', gap: 'var(--gap-sm)' }}
                        >
                          <ShieldAlert size={14} strokeWidth={1.8} />
                          <Text
                            runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}-title`}
                            style={aiPromptCardSectionTextStyle}
                          >
                            {approval.title}
                          </Text>
                        ...
                        <Badge
                          runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}-status`}
                          style={aiPromptApprovalStatusBadgeStyle}
                        >
                          {approval.status}
                        </Badge>
                      </Box>
                      <Text
                        runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}-command`}
                        style={aiPromptApprovalCommandStyle}
                      >
                        {approval.command}
                      </Text>
                      <Text
                        runaComponent={`ai-prompt-card-${prompt.id}-approval-${approval.id}-scope`}
                        style={aiPromptSubtitleStyle}
                      >
                        {approval.scope}
                      </Text>
                    </Box>
            ...
            <Box
              runaComponent={`ai-prompt-card-${prompt.id}-summary-section`}
              style={aiPromptSummarySectionStyle}
            >
              <Box
                runaComponent={`ai-prompt-card-${prompt.id}-summary-section-header`}
                style={aiPromptSectionHeaderStyle}
              >
                <Text
                  runaComponent={`ai-prompt-card-${prompt.id}-summary-label`}
                  style={aiPromptCardSectionLabelStyle}
                >
                  Summary
                </Text>
              ...
              <Text
                runaComponent={`ai-prompt-card-${prompt.id}-summary-text`}
                style={aiPromptCardSectionTextStyle}
              >
